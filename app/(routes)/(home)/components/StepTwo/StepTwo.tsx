...
export function StepTwo() {
  ...
  const handleSeletedPlatform = (platform: string) => {
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-x-8 gap-y-4 mt-4">
        {
          stepTwoData.map(({ icon, name }) => (
            <div
              key={name}
              className={`flex flex-col gap-1 items-center rounded-lg py-3 hover:via-violet-300 transition-all duration-300 cursor-pointer ${selectedPlatforms.includes(name) ? "bg-violet-900 text-white" : "bg-slate-100 text-violet-900"}`}
              onClick={() => handleSeletedPlatform(name)}
            >
