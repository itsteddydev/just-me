...
import { dataStepFourImage } from "./StepFour.data";
    ...
    <div>
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-5 mt-4 items-center">
        {dataStepFourImage.map(({ src }) => (
          ...
          >
            <Image src={src} alt="profile" className="h-30 w-30 rounded-full object-cover aspect-square" width={300} height={300} />
          </div>
        ...
        {
          photoUrl && (
            <div className={`flex flex-col items-center gap-2 p-1 rounded-full text-white transition-all duration-300 cursor-pointer ${selectedPhoto === photoUrl
              ? "bg-violet-500"
              : "hover:bg-violet-300"
              }`}
              onClick={() => { }}
            >
              <Image src={photoUrl} alt="profile" className="h-30 w-30 rounded-full object-cover aspect-square" width={300} height={300} />
            </div>
          )
        }
        {showUploadPhoto ? (
      ...
      <div className="mt-6 md:mt-16">
        <Button className="w-full bg-purple-600" onClick={() => { }}>Continue</Button>
      </div>
